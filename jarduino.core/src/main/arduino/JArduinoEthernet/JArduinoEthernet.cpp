...
// Constructor /////////////////////////////////////////////////////////////////
JArduinoEthernet::JArduinoEthernet() {
	state = RCV_WAIT;
	buffer_idx = 0;
}

// Public Methods //////////////////////////////////////////////////////////////
void JArduinoEthernet::init_JArduino(void) {
	// init the serial port
	Serial.begin(9600);
}

void JArduinoEthernet::poll_JArduino(void) {	
  uint8_t data;
...
void JArduinoEthernet::senddigitalReadResult(uint8_t value) {
...
void JArduinoEthernet::sendanalogReadResult(uint16_t value) {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
...
void JArduinoEthernet::sendpong() {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
...
void JArduinoEthernet::sendinterruptNotification(uint8_t interrupt) {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
   payload[3] = 1; // length of the params
...
void JArduinoEthernet::sendeeprom_value(uint8_t value) {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
   payload[3] = 1; // length of the params
...
void JArduinoEthernet::sendeeprom_write_ack() {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
   payload[3] = 0; // length of the params
...
// Private Methods /////////////////////////////////////////////////////////////
void JArduinoEthernet::parseIncommingMessage(uint8_t data[], uint8_t size) {
